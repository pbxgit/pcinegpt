/*
================================================================
APP.JS - DEFINITIVE BUG-FIX & STABILITY PASS
- Re-architects initialization for guaranteed stability and element availability.
- Implements a robust, centralized event delegation model for all user interactions.
- Perfects the mobile search overlay functionality and all associated event handling.
- Hardens rendering logic to ensure a consistently smooth and error-free UX.
================================================================
*/
...
async function renderHomeView() {
    appRoot.innerHTML = `<div class="view home-view"><div class="hero-section"><h1 class="title">Discover Your Next Obsession.</h1><p class="subtitle reveal-on-scroll">AI-powered recommendations for movies and shows, tailored to your unique taste.</p></div><div class="carousel-placeholder">${renderLoadingSkeletons(6)}</div></div>`;
    try {
    ...
    } catch (error) { console.error('Error fetching homepage carousels:', error); const placeholder = appRoot.querySelector('.carousel-placeholder'); if (placeholder) placeholder.innerHTML = renderError('Could not load movie carousels.'); }
}
async function renderSearchView(query) {
    appRoot.innerHTML = `<div class="view search-view"><div class="hero-section"><p class="subtitle">Results for</p><h1 class="title">“${query}”</h1></div><div class="search-results-grid">${renderLoadingSkeletons(10)}</div></div>`;
    ...
    } catch (error) { console.error('Error during AI search:', error); if (resultsGrid) resultsGrid.innerHTML = `<p class="subtitle" style="text-align: center;">${error.message}</p>`; }
}
