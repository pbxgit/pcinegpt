/*
================================================================
APP.JS - DEFINITIVE STABILITY & ARCHITECTURAL FIX V2
- Re-architects the application startup and rendering lifecycle to be completely
  linear and predictable, ELIMINATING ALL RACE CONDITIONS.
- The router is now wrapped in a top-level try/catch, making the entire
  application resilient to unexpected rendering errors.
...
// --- 6. VIEW RENDERING (ROBUST ATOMIC PATTERN) ---
async function renderHomeView() {
    ...
    try {
        ...
        viewHTML = `<div class="hero-section"><h1 class="title">Discover Your Next Obsession.</h1><p class="subtitle reveal-on-scroll">AI-powered recommendations for movies and shows, tailored to your unique taste.</p></div>${carouselsHTML}`;
    } catch (error) {
        console.error('Error fetching homepage carousels:', error);
        viewHTML = renderError('Could not load movie carousels.');
    }
...
async function renderSearchView(query) {
    ...
    try {
        ...
        viewHTML = `<div class="hero-section"><p class="subtitle">Results for</p><h1 class="title">“${query}”</h1></div><div class="search-results-grid">${tmdbResults.map(renderPosterCard).join('')}</div>`;
    } catch (error) {
        console.error('Error during AI search:', error);
        viewHTML = `<div class="hero-section"><p class="subtitle">Results for</p><h1 class="title">“${query}”</h1></div><div style="text-align:center;">${renderError(error.message)}</div>`;
    }
...
async function renderDetailView(movieId) {
    ...
    try {
        ...
        viewHTML = renderError('Could not load movie details.');
        appRoot.innerHTML = `<div class="view detail-view">${viewHTML}</div>`;
    }
}
...
async function router() {
    try {
        ...
        appRoot.innerHTML = renderError("The application encountered a serious error.");
    }
}
