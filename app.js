...
- Contains a robust client-side router and view lifecycle management.
- Uses a single, delegated event handler for all dynamic UI.
================================================================
*/
            ...
            document.getElementById('app-root').innerHTML = `
                <div class="view stats-view">
                    <div class="stats-prompt">
                        ...
                        <button class="trakt-button" data-action="connect-trakt">Connect Trakt</button>
                    </div>
                </div>`;
            return;
        }
            ...
            document.getElementById('app-root').innerHTML = `
                <div class="view stats-view">
                    ...
                    <div class="stats-grid">
                        <div class="stat-card"><h2>Total Hours Watched</h2><p class="stat-highlight">${totalHours.toLocaleString()}</p><p class="stat-label">of cinematic adventures</p></div>
                        <div class="stat-card"><h2>Movies vs Shows</h2><p class="stat-highlight">${moviesWatched.toLocaleString()} <span class="stat-label">Movies</span></p><p class="stat-highlight">${showsWatched.toLocaleString()} <span class="stat-label">Shows</span></p></div>
                        <div class="stat-card"><h2>Top Genres</h2><ul class="bar-chart">${topGenres.map(genre => {
                            // BUG FIX: Prevent division by zero if maxCount is 0
                            const width = maxCount > 0 ? (genre.count / maxCount) * 100 : 0;
                            return `
                                <li class="bar-chart-row">
                                    <span class="bar-chart-label">${genre.name}</span>
                                    <div class="bar-chart-bar-container">
                                        <div class="bar-chart-bar" style="width: ${width}%;">${genre.count.toLocaleString()}</div>
                                    </div>
                                </li>`;
                        }).join('')}</ul></div>
                    </div>
...
function createPosterCardHTML(movieData) {
    const { id, title, name, poster_path } = movieData;
...
function handleAppClick(event) {
    ...
    if (action === 'toggle-watchlist') {
        ...
        document.querySelectorAll(`[data-movie-id="${movieId}"]`).forEach(el => {
            if (el.dataset.action === 'toggle-watchlist') { // Handle both icons and buttons
...
function registerServiceWorker() { if('serviceWorker'in navigator)window.addEventListener('load',()=>{navigator.serviceWorker.register('/service-worker.js').then(e=>console.log('SW registered.',e)).catch(e=>console.error('SW reg failed:',e))})}
...
async function initialize() {
    ...
    document.getElementById('app-root').addEventListener('click', handleAppClick); // Use the new global handler
