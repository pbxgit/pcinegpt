...
const viewHandlers = {
    ...
    async renderDetailView({ param: id, type }) {
        ...
        const html = `
            <div class="view detail-view" data-media-id="${details.id}" data-media-type="${type}" data-media-title="${details.title || details.name}" data-media-year="${releaseYear}">
                <div class="detail-poster"><img src="${api.getPosterUrl(details.poster_path, 'w780')}" alt="${details.title || details.name}"></div>
    ...
    async renderSearchView({ param: query }) {
        ...
        let html = `<div class="view search-view"><h1 class="search-title">Results for "${decodedQuery}"</h1></div>`;
        ...
        let html = `
            <div class="view stats-view">
                ...
                <div class="stats-grid">
                    <div class="stat-card"><span>Movies Watched</span><p>${(movies.watched || 0).toLocaleString()}</p></div>
                    <div class="stat-card"><span>Shows Watched</span><p>${(shows.watched || 0).toLocaleString()}</p></div>
                    <div class="stat-card"><span>Episodes Watched</span><p>${(episodes.watched || 0).toLocaleString()}</p></div>
                    <div class="stat-card"><span>Total Time</span><p>${totalDays.toFixed(0)} <span class="unit">days</span></p></div>
                </div>
            </div>`;
        render(html);
    }
...
async function getTraktPersonalizedRecs() {
    ...
    const geminiResponse = await gemini.getAIRecommendations({ searchQuery: `Recommend 10 similar ${seedType}s to "${seedTitle}".` });
...
function handleSearch(query) {
    if (query?.trim()) window.location.hash = `#/search/${encodeURIComponent(query.trim())}`;
}

function bindSearchInputEvents() {
    const searchInput = document.getElementById('main-search-input');
    if (searchInput) searchInput.addEventListener('keydown', e => { if (e.key === 'Enter') handleSearch(e.target.value); });
    document.querySelectorAll('.chip').forEach(chip => chip.addEventListener('click', () => handleSearch(chip.dataset.query)));
}

async function handleWatchlistClick(e) {
    const button = e.target.closest('.watchlist-button');
    if (!button) return;
    if (!state.isTraktAuthenticated) { trakt.redirectToTraktAuth(); return; }
    ...
    button.disabled = true; button.innerHTML = 'Updating...';
    try {
        isInWatchlist ? await trakt.removeFromWatchlist(mediaItem) : await trakt.addToWatchlist(mediaItem);
        ...
        if (window.lucide) lucide.createIcons();
    } catch (error) { console.error("Failed to update watchlist:", error); button.innerHTML = 'Error'; }
...
function updateAuthUI() {
    ...
    dom.trakt.authBtn.textContent = state.isTraktAuthenticated ? 'Logout' : 'Connect Trakt';
    dom.trakt.statsLink.style.display = state.isTraktAuthenticated ? 'inline-block' : 'none';
}
...
function initEventListeners() {
    ...
    dom.trakt.authBtn.addEventListener('click', () => { state.isTraktAuthenticated ? trakt.logoutTrakt() : trakt.redirectToTraktAuth(); });
